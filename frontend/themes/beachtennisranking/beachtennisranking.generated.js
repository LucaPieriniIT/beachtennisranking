...
window.Vaadin.Flow['_vaadintheme_beachtennisranking_globalCss'] = window.Vaadin.Flow['_vaadintheme_beachtennisranking_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_beachtennisranking_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_beachtennisranking_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_beachtennisranking_globalCss'].push(target);
  }
  if (!document['_vaadintheme_beachtennisranking_componentCss']) {
    
    document['_vaadintheme_beachtennisranking_componentCss'] = true;
  }
